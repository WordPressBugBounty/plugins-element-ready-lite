...
class Reusable_Block
{
    ...
    public function block_support($prev, $post)
    {
        ...
        global $pagenow;
        if (($pagenow == 'post-new.php' && isset($_GET['post_type']) && sanitize_text_field(wp_unslash($_GET['post_type'])) == 'wp_block')  || ($pagenow == 'post.php' && isset($_GET['post']) && get_post_type(sanitize_text_field(wp_unslash($_GET['post']))) == 'wp_block')) {
            return true;
        }
        return $prev;
    }
