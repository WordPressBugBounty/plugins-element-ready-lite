...
class Menu_Settings
{
    ...
    public function ajax_megamenu_activate()
    {
        ...
        if (!wp_verify_nonce(sanitize_text_field(wp_unslash($_REQUEST['nonce'] ?? '')), 'element_ready_mega_menu_metabox_nonce')) {
            exit("No naughty business please");
        }

        $activation_status = sanitize_text_field(wp_unslash($_REQUEST['activation_status'] ?? ''));
    ...
    public function ajax_current_item_setting_update()
    {
        ...
        $item_id       = sanitize_text_field(isset($_REQUEST['item_id']) ? wp_unslash($_REQUEST['item_id']) : null);
        $is_mega_menu  = sanitize_text_field(isset($_REQUEST['is_mega_menu']) ? wp_unslash($_REQUEST['is_mega_menu']) : null);
    ...
    public function ajax_megamenu_activation_check()
    {
        ...
        $menu_id = isset($_REQUEST['menu_id']) ? sanitize_text_field(wp_unslash($_REQUEST['menu_id'])) : sanitize_text_field(wp_unslash($_REQUEST['menu_cur_id'] ?? ''));
    ...
    public function ajax_current_item_id_data()
    {
        ...
        $return_data = [];
        $item_id     = sanitize_text_field(wp_unslash($_REQUEST['menu_item_id'] ?? ''));
        if (is_admin() && defined('DOING_AJAX') && DOING_AJAX) {
...
?>
        <div id="element-ready-mega-menu-metabox">
            ...
            <p>
                <?php echo wp_kses_post(get_submit_button(esc_html__('Save', 'element-ready-lite'), 'element-ready-mega-menu-settings-save button-primary alignright', '', false)); ?>
