...
class Page_Settings extends BaseController
{
    ...
    function _nav_update($menu_id, $menu_item_db_id)
    {
        ...
        if (! isset($_POST['_element_ready_custom_menu_meta_nonce']) || ! wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['_element_ready_custom_menu_meta_nonce'])), 'element_ready_custom_menu_meta')) {
            return $menu_id;
        }

        //Mega menu  
        if (isset($_POST['element_ready_mega_menu_post_item_mega_menu_enable'][$menu_item_db_id])) {
            $sanitized_data = sanitize_text_field(wp_unslash($_POST['element_ready_mega_menu_post_item_mega_menu_enable'][$menu_item_db_id]));
        ...
        if (isset($_POST['element_ready_mega_menu_post_item_mega_menu_classic'][$menu_item_db_id])) {
            $sanitized_data = sanitize_text_field(wp_unslash($_POST['element_ready_mega_menu_post_item_mega_menu_classic'][$menu_item_db_id]));
        ...
        if (isset($_POST['_element_ready_menu_item_badge'][$menu_item_db_id])) {
            $sanitized_data = sanitize_text_field(wp_unslash($_POST['_element_ready_menu_item_badge'][$menu_item_db_id]));
        ...
        if (isset($_POST['_element_ready_menu_item_badge_color'][$menu_item_db_id])) {
            $sanitized_data = sanitize_text_field(wp_unslash($_POST['_element_ready_menu_item_badge_color'][$menu_item_db_id]));
        ...
        if (isset($_POST['_element_ready_menu_item_badge_bgcolor'][$menu_item_db_id])) {
            $sanitized_data = sanitize_text_field(wp_unslash($_POST['_element_ready_menu_item_badge_bgcolor'][$menu_item_db_id]));
        ...
        if (isset($_POST['_element_ready_menu_item_image'][$menu_item_db_id])) {
            $sanitized_data = sanitize_text_field(wp_unslash($_POST['_element_ready_menu_item_image'][$menu_item_db_id]));
...
?>
        <div class="description description-wide element-ready-fields-area" style="margin: 5px 0;">
            ...
            <?php endif ?>
            <span class="element-ready-menu-lebel"><?php esc_html_e("Menu Badge", 'element-ready-lite'); ?></span>
            <br />
            ...
            <span class="element-ready-menu-lebel"><?php esc_html_e("Badge color", 'element-ready-lite'); ?></span>
            <br />
            ...
            <span class="element-ready-menu-lebel"><?php esc_html_e("Badge background", 'element-ready-lite'); ?></span>
            <br />
            ...
            <span class="element-ready-menu-lebel"><?php esc_html_e("Image", 'element-ready-lite'); ?></span>
            <br />
