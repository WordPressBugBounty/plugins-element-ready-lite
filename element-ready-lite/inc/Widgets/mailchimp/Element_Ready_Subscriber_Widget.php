...
class Element_Ready_Subscriber_Widget extends Widget_Base
{
	...
	protected function render()
	{
		...
		if (!empty($settings['title_tag'])) {
			$allowed_tags = array('div', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span', 'p'); // List allowed HTML tags
			$title_tag = in_array($settings['title_tag'], $allowed_tags, true) ? $settings['title_tag'] : 'div';
		} else {
			$title_tag = 'div';
		}
		...
		if (!empty($settings['title'])) {
			$title = '<' . esc_attr($title_tag) . ' class="box__title">' . wp_kses_post(wpautop($settings['title'])) . '</' . esc_attr($title_tag) . '>';
		} else {
			$title = '';
		}
		...
		if ('yes' == $settings['show_button'] && !empty($settings['button_text'])) {
			$button = '<button ' . wp_kses_post($this->get_render_attribute_string('submit_button')) . '>' . esc_html($settings['button_text']) . '</button>';
		}

		// Button With Icon
		if (!empty($settings['button_icon'])) {
			if ('left' == $settings['button_icon_align']) {
				$button = '<button ' . wp_kses_post($this->get_render_attribute_string('submit_button')) . '><i class="subscribe__btn_icon_left ' . esc_attr($settings['button_icon']) . '"></i>' . esc_html($settings['button_text']) . '</button>';
			} elseif ('right' == $settings['button_icon_align']) {
				$button = '<button ' . wp_kses_post($this->get_render_attribute_string('submit_button')) . '>' . esc_html($settings['button_text']) . '<i class="subscribe__btn_icon_right ' . esc_attr($settings['button_icon']) . '"></i></button>';
			}
		}
		...
		echo '
			<div ' . wp_kses_post($this->get_render_attribute_string('subscriber-form-attr')) . '>				
				' . (isset($icon) ? $icon : '') . '
