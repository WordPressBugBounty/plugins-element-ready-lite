...
class Element_Ready_Area_Title_Widget extends Widget_Base
{
	...
	protected function render()
	{
		...
		if ('yes' == $settings['show_button'] && (!empty($settings['button_text']) && !empty($settings['button_link']))) {
			$button = wp_kses_post(sprintf('<a class="area__button %s" %s>%s</a>', esc_attr($button_animation), wp_kses_post($this->get_render_attribute_string('more_button')), esc_html($settings['button_text'])));
		}

		/*Button With Icon*/
		if (!empty($settings['button_icon'])) {
			if ('left' == $settings['button_icon_align']) {
				$button = wp_kses_post(sprintf('<a class="area__button %s" %s><i class="area__button_icon_left %s"></i>%s</a>', esc_attr($button_animation), wp_kses_post($this->get_render_attribute_string('more_button')), esc_attr($settings['button_icon']), esc_html($settings['button_text'])));
			} elseif ('right' == $settings['button_icon_align']) {
				$button = wp_kses_post(sprintf('<a class="area__button %s" %s>%s<i class="area__button_icon_right %s"></i></a>', esc_attr($button_animation), wp_kses_post($this->get_render_attribute_string('more_button')), esc_html($settings['button_text']), esc_attr($settings['button_icon'])));
			}
		}
