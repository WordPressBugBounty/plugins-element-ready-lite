...
class Element_Ready_Box_Widget extends Widget_Base
{
	...
	protected function register_controls()
	{
		...
		$this->add_control(
			'box_circles',
			[
				'label'        => esc_html__('Box Circles Effect', 'element-ready-lite'),
		...
		-----------------------------*/
		$this->start_controls_section(
			'box_circles_effect_style_section',
			[
				'label' => esc_html__('Box Circles Effect', 'element-ready-lite'),
				'tab'   => Controls_Manager::TAB_STYLE,
				'condition' => [
		...
		$this->add_control(
			'box_circles_active',
			[
				'label'        => esc_html__('Always Active', 'element-ready-lite'),
				'type'         => Controls_Manager::SWITCHER,
				'label_on'     => esc_html__('Show', 'element-ready-lite'),
				'label_off'    => esc_html__('Hide', 'element-ready-lite'),
				'return_value' => 'yes',
		...
		$this->add_control(
			'box_circles_offset',
			[
				'label'     => esc_html__('Position Offset', 'element-ready-lite'),
		...
		$this->add_responsive_control(
			...
			[
				...
				'selectors' => [
					'{{WRAPPER}} .circles-wrap' => 'left: {{SIZE}}{{UNIT}};right:auto;',
				],

			]
		);
		$this->add_responsive_control(
			'box_circles_from_right',
			[
				...
				'selectors' => [
					'{{WRAPPER}} .circles-wrap' => 'right: {{SIZE}}{{UNIT}};left:auto',
				],
			]
		);

		$this->add_responsive_control(
			'box_circles_from_top',
			[
				...
				'selectors' => [
					'{{WRAPPER}} .circles-wrap' => 'top: {{SIZE}}{{UNIT}};bottom:auto',
				],
			]
		);

		$this->add_responsive_control(
			'box_circles_from_bottom',
			[
				...
				'selectors' => [
					'{{WRAPPER}} .circles-wrap' => 'bottom: {{SIZE}}{{UNIT}};top:auto',
				],
			]
		);
		...
		$this->add_control(
			'box_circles_style',
			[
				'label'     => esc_html__('Background and Border', 'element-ready-lite'),
				'type'      => Controls_Manager::HEADING,
				'separator' => 'before',
			]
		);
		...
		$this->add_group_control(
			Group_Control_Background::get_type(),
			[
				'name' => 'box_circles_background',
				'label' => esc_html__('Background', 'element-ready-lite'),
				'types' => ['classic', 'gradient'],
				'separator'		=> 'before',
				'selector' => '{{WRAPPER}} .single__box .circle',
			]
		);
		$this->add_group_control(
			Group_Control_Border::get_type(),
			[
				'name'     => 'box_circles_border',
				'label'    => esc_html__('Border', 'element-ready-lite'),
				'selector' => '{{WRAPPER}} .single__box .circle',
			]
		);
	...
	protected function render()
	{
		...
		if (!empty($settings['button_link']['url'])) {
			...
			if (isset($settings['button_link']['is_external']) && !empty($settings['button_link']['is_external'])) {
				$this->add_render_attribute('more_button', 'target', '_blank');
			}
			if (isset($settings['button_link']['nofollow']) && !empty($settings['button_link']['nofollow'])) {
		...
		// box circles
		$hoverCirclesEfeect = (isset($settings['box_circles']) && $settings['box_circles'] === 'yes') ? true : false;
		if ($hoverCirclesEfeect) {
			$box_circles_active = $settings['box_circles_active'] ?? '';
			if ($box_circles_active === 'yes') {
				$this->add_render_attribute('box_wrap_style_attr', 'class', 'circles_active');
				$this->add_render_attribute('box_style_attr', 'class', 'circles_active');
			}
			$this->add_render_attribute('box_wrap_style_attr', 'class', 'has_box_circles');
			$this->add_render_attribute('box_style_attr', 'class', 'has_box_circles');
			$hoverCirclesEfeect = '<div class="circles-wrap">
				<div class="circles">
